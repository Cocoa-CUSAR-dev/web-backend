...
class HarvestAnalyticsService(
    ...
    fun getCumulativeSeries(filter: Analytics.Query.HarvestFilter): Analytics.Report.TimeSeries {
        val report = buildReport(filter, "Running Total Harvest Weight per Month", "kg") {
            harvestRepository.fetchMonthlyDelta(filter)
        }

        val cumulativeEntries = report.series.map { entry ->
            var runningTotal = 0.0
            entry.copy(values = entry.values.map {
    ...
    private fun buildReport(
        ...
        val isHierarchyConsistent = locationRepository.isHierarchyValid(
        ...
        val rawData = fetcher()
        val metadata = locationRepository.enrichHierarchy(
            provinceId = filter.provinceId,
            districtId = filter.districtId,
            subdistrictId = filter.subdistrictId,
            farmId = filter.farmId
        ...
        val dataMap = rawData.groupBy { it.gradeCode }.mapValues { (_, rows) ->
            rows.associate { java.time.YearMonth.from(it.month) to it.value }
        }

        val gradesToInclude = filter.gradeCode?.let {
            listOf(it)
        } ?: allGrades

        val series = gradesToInclude.map { grade ->
            val gradeMonths = dataMap[grade] ?: emptyMap()

            val values = monthRange.map { month ->
